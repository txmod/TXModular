...
TXFreqBpmMinMaxSldr {
	var <>labelView, <>labelView2, <>sliderView, <>numberView, <>bpmNumberView, <>rangeView, <>minNumberView;
	var <>maxNumberView, <>presetPopupView, <>controlSpec, controlSpec2, <>action, viewValue, <>round = 0.0001;
	...
	*new { arg window, dimensions, label, controlSpec, action, initVal,
	...
	init { arg window, dimensions, label, argControlSpec, argAction, initVal,
		...
		sliderView.action = {
			...
			bpmNumberView.value = 60 * viewValue;
			action.value(this);
		};
		...
		bpmNumberView.action = {
			numberView.valueAction = controlSpec2.constrain(bpmNumberView.value / 60);
			bpmNumberView.value = 60 * viewValue;
		};
