...
TXPingPongSt : TXModuleBase {		// PingPong Delay stereo in, stereo out
	...
	*initClass{
		...
		classData.defaultName = "Ping Pong St";
	...
	*new{ arg argInstName;
	...
	init {arg argInstName;
		...
		synthDefFunc = {
			...
			feedbackVal = feedbackMin + ( (feedbackMax-feedbackMin) *
				(feedback + modFeedback + (freezeCombined * 2)).max(0).min(1) );
