...
TXLoopPlayerSt2 : TXModuleBase {
	...
	*initClass{
		...
		classData.defaultName = "Loop Player St";
		...
		classData.arrBufferSpecs = [ ["bufnumLoopStereo", 2048, 2],  ["bufnumLoopMono", 2048, 1],
			["bufnumFeel1", 512, 1], ["bufnumFeel2", 512, 1], ["bufnumGroove1", 512, 1], ["bufnumGroove2", 512, 1], ];
	...
	*reloadAllLoops{arg argbankNo, argindex;
	...
	init {arg argInstName;
		...
		arrOptionData = [
			[ // 0 - loop type indexing function
				["Looped - play while gate is on",
					{arg outRate, bufnumLoopStereo, bufnumLoopMono, sStart, sEnd;
						if (this.getSynthArgSpec("sampleIsStereo") == 1, {
							Phasor.ar(0, outRate * SampleDur.ir/ ((sEnd - sStart).max(0.00001) * BufDur.kr(bufnumLoopStereo)));
						}, {
							Phasor.ar(0, outRate * SampleDur.ir/ ((sEnd - sStart).max(0.00001) * BufDur.kr(bufnumLoopMono)));
						});
					}
				],
				["Single shot - play while gate is on",
					{arg outRate, bufnumLoopStereo, bufnumLoopMono, sStart, sEnd;
						var outSig;
						if (this.getSynthArgSpec("sampleIsStereo") == 1, {
							outSig = Sweep.ar(1, outRate / ((sEnd - sStart).max(0.00001)
								* BufDur.kr(bufnumLoopStereo)));
						}, {
							outSig = Sweep.ar(1, outRate / ((sEnd - sStart).max(0.00001)
								* BufDur.kr(bufnumLoopMono)));
						});
			...
			[// 1 - Time-stretching
				["Off", {arg bufnumLoopStereo, bufnumLoopMono, bufIndex;
					var output;
					if (this.getSynthArgSpec("sampleIsStereo") == 1, {
						output = BufRd.ar(2, bufnumLoopStereo, bufIndex * BufFrames.kr(bufnumLoopStereo), 0);
					}, {
						output = BufRd.ar(1, bufnumLoopMono, bufIndex * BufFrames.kr(bufnumLoopMono), 0).dup;
					});
					output;
				}],
				["Granular time-stretch", {
					arg bufnumLoopStereo, bufnumLoopMono, bufIndex, freqScale, windowSize, overlaps, randomness, interpolation;
					var output;
					if (this.getSynthArgSpec("sampleIsStereo") == 1, {
						output = Warp1.ar(2, bufnumLoopStereo, bufIndex, freqScale, windowSize,
							-1, overlaps, randomness, interpolation);
					}, {
						output = Warp1.ar(1, bufnumLoopMono, bufIndex, freqScale, windowSize,
							-1, overlaps, randomness, interpolation).dup;
					});
					output;
				}],
				["Granular time-stretch with zero-crossing search (best)", {
					...
					var output;
					if (this.getSynthArgSpec("sampleIsStereo") == 1, {
						output = WarpZ.ar(2, bufnumLoopStereo, bufIndex, freqScale, windowSize,
							-1, overlaps, randomness, interpolation, 0.02, 0.1);
					}, {
						output = WarpZ.ar(1, bufnumLoopMono, bufIndex, freqScale, windowSize,
							-1, overlaps, randomness, interpolation, 0.02, 0.1).dup;
					});
					output;
				}],
			],
		...
		synthDefFunc = {
			...
			bufnumLoopStereo, bufnumLoopMono, bufnumFeel1, bufnumFeel2, bufnumGroove1, bufnumGroove2, sampleIsStereo,
			...
			feelMulti, feelMorph, feelMorphMin, feelMorphMax,
			grooveMulti, grooveMorph, grooveMorphMin, grooveMorphMax,
			...
			windowSize, overlaps, randomness, interpolation, freqScale, indScaled, minInd, lookupInd;
			...
			outLoop = outFunction.value(bufnumLoopStereo, bufnumLoopMono, grooveScaled, freqScale, windowSize,
		...
		arrActionSpecs = this.buildActionSpecs([
			...
			["TXListViewActionPlusMinus", "Loop", {["No Loop"]++system.loopFileNames(bankNo, true)},
	...
	bufferStore { arg argIndex, argArray;
		buffers.at(2 + argIndex).sendCollection(argArray);
	}
