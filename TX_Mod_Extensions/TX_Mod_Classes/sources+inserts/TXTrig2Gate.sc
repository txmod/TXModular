	...
	*new{ arg argInstName;
	...
	init {arg argInstName;
		...
		arrSynthArgSpecs = [
			["in", 0, 0],
			["out", 0, 0],
			["holdTime", ControlSpec(0.01, 5, \exp).unmap(0.3), defLagTime],
			["holdTimeMin", 0.01, defLagTime],
			["holdTimeMax", 5, defLagTime],
		...
		synthDefFunc = { arg in, out, holdTime, holdTimeMin, holdTimeMax, modHoldTime = 0;
			var inTrig, holdTimeSum, gateFunc, outGate;
			inTrig = TXClean.kr(In.kr(in, 1));
			holdTimeSum = (holdTime + modHoldTime).linexp(0.01, 1.0, holdTimeMin, holdTimeMax, \minmax);
			gateFunc = this.getSynthOption(0);
			outGate = gateFunc.value(holdTimeSum, inTrig);
		...
		guiSpecArray = [
			...
			["SpacerLine", 10],
			["TXMinMaxSliderSplit", "Hold time", holdControlSpec, "holdTime", "holdTimeMin", "holdTimeMax"],
		];
