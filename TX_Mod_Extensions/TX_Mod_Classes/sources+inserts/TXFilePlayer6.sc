...
TXFilePlayer6 : TXModuleBase {	// Disk File Player
	...
	*initClass{
		...
		classData.defaultName = "File Player";
		...
		classData.arrBufferSpecs = [ ["bufnumSound", 32768, 1] ];
	...
	*stopAllPlayers {
	...
	init {arg argInstName;
		...
		synthDefFunc = {
			...
			outSound = VDiskIn.ar(1, bufnumSound, playRateCombined, loop) * outLevel;
		...
		arrActionSpecs = this.buildActionSpecs([
			...
			["NextLine"],
			["TXStaticText", "File name",
				{this.sampleFileName}, {arg view; sampleFileNameView = view}],
			["commandAction", "Play", {
				moduleNode.release(0);
