...
TXWaveform5 : TXModuleBase {
	...
	*initClass{
		...
		classData.defaultName = "Waveform";
	...
	*new{ arg argInstName;
	...
	init {arg argInstName;
		...
		synthDefFunc = {
			arg inmodulator, out, freq, freqMin, freqMax, change1, change1Min, change1Max, change2, change2Min, change2Max,
			...
			modFreq = 0, modChange1 = 0, modChange2 = 0, modFreqSelector = 0,
			modLag = 0, modLag2 = 0, modFMDepth=0, modFMFreq=0, modFMRatio=0;

			var outFreqFunc, outFreq, outFreqLag, waveFunction, outChange1, outChange2, lagtime, lagtime2;
			var freqModFunc, fmOut, outLagFunction, outVol, outWave, ampCompFunction, outAmpComp;
			...
			outWave = waveFunction.value((outFreqLag * fmOut).max(0.midicps), outChange1, outChange2);
			ampCompFunction = this.getSynthOption(3);
			outAmpComp = ampCompFunction.value(outFreqLag);
			Out.ar(out, TXClean.ar(outVol * outWave * outAmpComp));
		};
