...
TXPhaser : TXModuleBase {
	...
	*initClass{
		...
		classData.arrBufferSpecs = [
			["bufnumDelay", defSampleRate * classData.maxDelaytime, 1],
			["bufnumDelay2", defSampleRate * classData.maxDelaytime * 0.5, 1] // for input delay
		];
	}
	...
	*arrLFOFreqRanges {
	...
	init {arg argInstName;
		...
		synthDefFunc = {
			arg in, out, bufnumDelay, bufnumDelay2, delay=0.1, delayMin, delayMax,
			...
			var input, inDelayed, outSound, delaytime, delaytimeMod, feedbackVal, decaytime, mixCombined;
			...
			inDelayed = BufDelayC.ar(bufnumDelay2, input, delaytime * 0.5);
			outSound = BufAllpassC.ar(bufnumDelay, input, delaytimeMod, decaytime,
				mixCombined, inDelayed * (1-mixCombined));
