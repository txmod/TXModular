...
TXFreqShiftSt : TXModuleBase {
	...
	*initClass{
		...
		classData.defaultName = "Freq Shift St";
	...
	*new{ arg argInstName;
	...
	init {arg argInstName;
		...
		arrSynthArgSpecs = [
			...
			["stereoShiftMin", -1000, defLagTime],
			["stereoShiftMax", 1000, defLagTime],
			["fmDepth", 0.2, defLagTime],
		...
		synthDefFunc = {
			arg in, sideChain, out, freqShift, freqShiftMin, freqShiftMax, stereoShift, stereoShiftMin, stereoShiftMax,
			fmDepth, fmDepthMin, fmDepthMax, wetDryMix,
			modFreqShift = 0, modStereoShift=0, modFMDepth = 0, modWetDryMix = 0;

			var input, sideChainSig, sFreqShift, sStereoShift, sfmDepth, sPhase, outSound, mixCombined;
			...
			sStereoShift = stereoShiftMin + ((stereoShiftMax - stereoShiftMin) * (stereoShift + modStereoShift).max(0).min(1));
			sStereoShift = sStereoShift;
			outSound = FreqShift.ar(input, sFreqShift + [sStereoShift, sStereoShift.neg], sPhase);
		...
		guiSpecArray = [
			...
			["SpacerLine", 6],
			["TXMinMaxSliderSplit", "Stereo Shift", classData.freqShiftSpec, "stereoShift", "stereoShiftMin", "stereoShiftMax",
				nil, classData.shiftRangePresets],
			["NextLine"],
			["DividingLine"],
			["SpacerLine", 10],
