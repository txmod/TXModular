...
// NOTE - Sung formant data taken from the SuperCollider Quark Vowel (2010 - 2012, Florian Grond and Till Bovermann)
// which used formant data taken from the Csound manual:
// http://ecmc.rochester.edu/onlinedocs/Csound/Appendices/table3.html

TXVowelMorph : TXModuleBase {
	...
	*initClass{
		...
		classData.defaultName = "Vowel Morph";
		classData.moduleRate = "audio";
		classData.moduleType = "source";
		classData.arrAudSCInBusSpecs = [
			["External Input", 1, "extInput"]
		];
	...
	*new{ arg argInstName;
	...
	init {arg argInstName;
			...
			[
				...
				["External Input",
					{arg baseFreq, arrFreqs, arrWidths, arrAmps, extInput;
						var sound = TXClean.ar(InFeedback.ar(extInput, 1));
						2 * // scale
						Mix.new(
							arrFreqs.collect({arg freq, i;
								var holdFreq = classData.freqSpec.map(freq);
		...
		synthDefFunc = {
			...
			outsound = startEnv * soundFunc.value(outFreqLag, arrFreqs, arrWidths, arrAmps, extInput);
