...
TXStepSequencer : TXModuleBase {
	...
	*initClass {
		...
		classData.defaultName = "Step Sequencer";
	...
	*stopAllSequencers {
	...
	init {arg argInstName;
		...
		synthDefFunc = { arg out, seqOn, modSeqBPM=0;
		...
		guiSpecTitleArray = [
			...
			["TXStaticText", "Status", {this.runningStatus},
				{arg view; if ((view.class == TextView) || (view.class == TextField), {runningStatusView = view},
					{runningStatusView = view.textView}); }, 110, 40, TXColor.paleYellow],
			["Spacer", 3],
		...
		arrActionSpecs = this.buildActionSpecs([
			...
			["SeqPlayRange", "seqStartStep", "seqEndStep", nil, 64, "Play steps",
				{this.updateSlot; this.resetSequencer; this.updatePatternTime; system.showViewIfModDisplay(this);}],
	...
	buildGuiSpecArray {
		...
		holdValidOptions = ["showOutputs", "showBPMOptions", "showPatterns", "showSteps",
		...
		guiSpecArray = [
			...
			["Spacer", 1],
			["ActionButton", "Steps", {this.setSynthArgSpec("displayOption", "showSteps");
				this.buildGuiSpecArray; system.showView;}, 100,
			TXColor.white, this.getButtonColour(this.getSynthArgSpec("displayOption") == "showSteps")],
			["Spacer", 1],
		...
		if (this.getSynthArgSpec("displayOption") == "showOutputs", {
			...
			guiSpecArray = guiSpecArray ++[
				...
				["SpacerLine", 2],
				["TextBarLeft", "Polyphonic modules to be triggered directly by the sequencer:"],
		...
		if ((seqRunning == true) or: (triggerModeRunning == true)
			...
			and: (   this.getSynthArgSpec("displayOption") == "showPatterns"
				or: (this.getSynthArgSpec("displayOption") == "showSteps")
				...
				// if not playing a Pattern Chain
				// showSteps
				if (this.getSynthArgSpec("displayOption") == "showSteps", {
					...
					guiSpecArray = guiSpecArray ++[
						[
							...
							["TXCheckBox", "MIDI learn notes & velocities ",
								"midiLearn", {arg view; this.midiLearn(view.value);}, 200],
						...
						//			[
						//				["TXPopupActionPlusMinus", "Chord/ scale", TXScale.arrScaleNames, "xxxxxxx", nil, 400],
						//				["NextLine"],
				...
				if (this.getSynthArgSpec("displayOption") == "showPatterns", {
					guiSpecArray = guiSpecArray ++[
						...
						["DividingLine"],
						["SpacerLine", 2],
						["TXTextBox", "Pattern desc.", "description", nil, 400],
						["SpacerLine", 2],
						["DividingLine"],
						["SpacerLine", 2],
						["TXNumberPlusMinus", "Pattern slot", ControlSpec(0, 99, step: 1), "slotNo",
						...
						["DividingLine"],
						["SpacerLine", 2],
						["TXPopupActionPlusMinus", "Next pattern",
					...
					if (this.getSynthArgSpec("nextPatternInd") > 1, {
						guiSpecArray = guiSpecArray ++ [
							...
							// 	{this.stopSequencer; this.prepareToStartSeq;}
							// ],
							["TextBar", "Pattern Chain", 80, 30, nil, nil, \right],
							["Spacer", 10],
							["TXNumberPlusMinus", "Start step", ControlSpec(1, 64, 'lin', 1), "chainStartStep", {
								var holdCurrStep;
								this.setSynthArgSpec("displayFirstChainStep", 0);
								this.constrainChainStep;
								holdCurrStep = this.getSynthArgSpec("chainCurrentStep");
								this.stopSequencer;
								this.prepareToStartSeq;
								this.setSynthArgSpec("chainCurrentStep", holdCurrStep);
								this.updateSlotNo;
								system.showView; }
							],
							["Spacer", 10],
							["TXNumberPlusMinus", "End step", ControlSpec(1, 64, 'lin', 1), "chainEndStep", {
								var holdCurrStep;
								this.setSynthArgSpec("displayFirstChainStep", 0);
								this.constrainChainStep;
								holdCurrStep = this.getSynthArgSpec("chainCurrentStep");
							...
							["SpacerLine", 1],
							["TextBarLeft",
								"   Click on a Chain step button number to load the Pattern in the slot"
								++ " (yellow highlight)", 470],
							["NextLine"],
				...
				if (this.getSynthArgSpec("displayOption") == "showProcesses", {
					guiSpecArray = guiSpecArray ++[
						...
						["DividingLine"],
						["SpacerLine", 2],
						["TXListViewActionPlusMinus", "Process", arrProcessSpecs.collect({arg item, i; item.at(0);}), "processTypeInd",
							{this.buildGuiSpecArray; system.showView;}, 410, 80],
	...
	updateScrollOrigin {arg sourceView, argOrigin;
		holdVisibleOrigin = argOrigin;
		arrScrollViewsH.asArray.do({arg item, i;
			if (item.notNil and: {item != sourceView}, {item.visibleOrigin = (argOrigin.x @ 0);}) });
		arrScrollViewsV.asArray.do({arg item, i;
			if (item.notNil and: {item != sourceView}, {item.visibleOrigin = (0 @ argOrigin.y);}) });
		arrScrollViewsVH.asArray.do({arg item, i;
			if (item.notNil and: {item != sourceView}, {item.visibleOrigin = (argOrigin.x @ argOrigin.y);}) });
	}

	updateScrollOriginX {arg sourceView, argOrigin;
		// this version only uses X value passed
		holdVisibleOrigin.x = argOrigin.x;
		this.updateScrollOrigin(sourceView, holdVisibleOrigin);
	}
	...
	oscControlActivate {
		...
		oscControlResp = OSCFunc({arg msg, time, addr,  recvPort;
			if (msg[2] == sendTrigID,{
				holdModSeqBPM = msg[3];
				this.setTempo;
			});
	...
	sendMIDIMessage { arg outNote, outVel, gateTime, outControl1, outControl2, outControl3, delay = 0;
		var portInd, channel, midiLoopBack;
		...
		if (midiLoopBack == 1, {
			if (this.getSynthArgSpec("midiNoteOn") == 1, {
				SystemClock.sched( delay,{
					MIDIIn.doNoteOnAction(0, channel.asInteger, outNote.asInteger, outVel);
					nil;
				});
				SystemClock.sched( delay + gateTime,{
					MIDIIn.doNoteOffAction(0, channel.asInteger, outNote.asInteger, 0);
					nil;
				});
			});
			// "Controller 1"
			if (this.getSynthArgSpec("midiControl1On") == 1, {
				SystemClock.sched( delay,{
					MIDIIn.doControlAction(0, channel.asInteger, this.getSynthArgSpec("midiControlNo1").asInteger, outControl1 );
					nil;
				});
			});
			// "Controller 2"
			if (this.getSynthArgSpec("midiControl2On") == 1, {
				SystemClock.sched( delay,{
					MIDIIn.doControlAction(0, channel.asInteger, this.getSynthArgSpec("midiControlNo2").asInteger, outControl2 );
					nil;
				});
			});
			// "Controller 3"
			if (this.getSynthArgSpec("midiControl3On") == 1, {
				SystemClock.sched( delay,{
					MIDIIn.doControlAction(0, channel.asInteger, this.getSynthArgSpec("midiControlNo3").asInteger, outControl3 );
					nil;
				});
			});
		});
		if (portInd > 0, {
			...
			if (this.getSynthArgSpec("midiNoteOn") == 1, {
				SystemClock.sched( delay,{
					holdMIDIOutPort.noteOn(channel.asInteger, outNote.asInteger, outVel);
					nil;
				});
				SystemClock.sched( delay + gateTime,{
					holdMIDIOutPort.noteOff(channel.asInteger, outNote.asInteger, 0);
					nil;
				});
			});
			// "Controller 1"
			if (this.getSynthArgSpec("midiControl1On") == 1, {
				SystemClock.sched( delay,{
					holdMIDIOutPort.control(channel.asInteger, this.getSynthArgSpec("midiControlNo1").asInteger, outControl1 );
					nil;
				});
			});
			// "Controller 2"
			if (this.getSynthArgSpec("midiControl2On") == 1, {
				SystemClock.sched( delay,{
					holdMIDIOutPort.control(channel.asInteger, this.getSynthArgSpec("midiControlNo2").asInteger, outControl2);
					nil;
				});
			});
			// "Controller 3"
			if (this.getSynthArgSpec("midiControl3On") == 1, {
				SystemClock.sched( delay,{
					holdMIDIOutPort.control(channel.asInteger, this.getSynthArgSpec("midiControlNo3").asInteger, outControl3);
					nil;
				});
			});
		});
	...
	playNextStep { arg argMuteStep = 0;
		...
		if (	// if step is to be played
			...
			and: (argMuteStep == 0),
			{
				...
				system.server.makeBundle(system.seqLatency + outDelay, {
				...
				system.server.makeBundle(system.seqLatency + outDelay + 0.01, {
					// write note & velocity as control values to output busses
					outBus.setAt(5, 0);
				});
	...
	loadExtraData {arg argData;
		...
		if (system.dataBank.savedSystemRevision < 1002, {
			oldInd = this.getSynthArgSpec("procScaleTypeInd");
			newInd = TXScale.convertPreRev1002Ind(oldInd);
			this.setSynthArgSpec("procScaleTypeInd", newInd);
		});
		this.buildGuiSpecArray;
		this.resetSequencer;
		this.restoreOutputs;
	...
	getSlotData {
		var holdData;
		holdData = ["seqStartStep", "seqEndStep", "seqNoteBase", "arrNotes", "arrRandOctaves",
	...
	constrainChainStep {
		...
		var holdCurrent, holdStart, holdEnd, newStart, newEnd;
		...
		newStart = min(holdStart, holdEnd);
		newEnd = max(holdStart, holdEnd);
		this.setSynthArgSpec("chainStartStep", newStart);
		this.setSynthArgSpec("chainEndStep", newEnd);
		this.setSynthArgSpec("chainCurrentStep", holdCurrent.max(newStart).min(newEnd));
	}
		...
		arrProcessSpecs = [
			...
			["Constrain notes to chord, mode, scale",
				...
				[
					...
					["NextLine"],
					["SpacerLine", 1],
					["TXListViewActionPlusMinus", "Key / root",
						["C", "C#", "D", "D#", "E","F", "F#", "G", "G#", "A", "A#", "B"],
						"procScaleRoot", nil, 200, 180],
					["Spacer", 20],
					["TXListViewActionPlusMinus", "Chord/ scale", TXScale.arrScaleNames, "procScaleTypeInd", nil, 400, 180],
				],
			],
			["Generate notes from chord, mode, scale",
				...
				[
					...
					["TXListViewActionPlusMinus", "Key / root",
						["C", "C#", "D", "D#", "E","F", "F#", "G", "G#", "A", "A#", "B"],
						"procScaleRoot", nil, 200, 130],
					["Spacer", 20],
					["TXListViewActionPlusMinus", "Chord/ scale", TXScale.arrScaleNames, "procScaleTypeInd", nil, 400, 130],
					["SpacerLine", 1],
					["TXPopupActionPlusMinus", "Note order", [ "random order", "random selection & order",
	...
	updateProcTarget {arg argSynthArgString, argChangeArray;
		var holdArray, outArray;
		holdArray = argChangeArray.deepCopy;
		outArray = this.getSynthArgSpec(argSynthArgString);
		targetSize.do({ arg i;
			outArray.put(loTargetIndex + i, holdArray.at(i));
		});
