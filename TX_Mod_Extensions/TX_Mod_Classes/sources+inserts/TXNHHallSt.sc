...
TXNHHallSt : TXModuleBase {
	...
	*initClass{
		...
		classData.defaultName = "NHHall St";
		...
		classData.arrCtlSCInBusSpecs = [
			...
			["Stereo Blend", 1, "modStereoBlend", 0],
		...
		classData.arrBufferSpecs = [
			["bufnumDelayL", defSampleRate * 5, 1],
			["bufnumDelayR", defSampleRate * 5, 1]
		];
	...
	*new{ arg argInstName;
	...
	init {arg argInstName;
		...
		arrSynthArgSpecs = [
			...
			["stereoBlend", 0.5, defLagTime],
			["stereoBlendMin", 0, defLagTime],
			["stereoBlendMax", 1, defLagTime],
		...
		synthDefFunc = {
			arg in, out, bufnumDelayL, bufnumDelayR, preDelay, preDelayMin, preDelayMax,
			reverbTime, reverbTimeMin, reverbTimeMax, infinite,
			stereoBlend, stereoBlendMin, stereoBlendMax, lowFreq, lowRatio, lowRatioMin, lowRatioMax,
			...
			modPreDelay=0, modReverbTime=0, modInfinite=0, modStereoBlend=0, modLowRatio=0, modHighRatio=0,
			...
			var sum_preDelay, sum_reverbTime, sumInfinite, sum_stereoBlend, sum_lowRatio, sum_highRatio;
			...
			sum_stereoBlend = (stereoBlend + modStereoBlend).max(0).min(1)
				.linlin(0, 1, stereoBlendMin, stereoBlendMax);
			...
			outHall = NHHall.ar(input, sum_reverbTime + sumInfinite, sum_stereoBlend,
				lowFreq, sum_lowRatio, highFreq, sum_highRatio,  sum_earlyDiffusion, sum_lateDiffusion,
				sum_modulationDepth, sum_modulationRate);
			delayedHall = [
				BufDelayC.ar(bufnumDelayL, outHall[0], sum_preDelay),
				BufDelayC.ar(bufnumDelayR, outHall[1], sum_preDelay),
			];
		...
		arrActionSpecs = this.buildActionSpecs([
			...
			["TXMinMaxSliderSplit", "Stereo blend", ControlSpec(0, 1), "stereoBlend",
				"stereoBlendMin", "stereoBlendMax"],
			["WetDryMixSlider"],
