...
TXNoiseGateSt : TXModuleBase {
	...
	*initClass{
		...
		classData.defaultName = "Noise Gate St";
	...
	*new{ arg argInstName;
	...
	init {arg argInstName;
		...
		arrOptionData = [
			...
			[ // 5 - Channel link
				//["Linked Left & Right channels", {arg input; Mix.ar(input)}], // OLD
		...
		synthDefFunc = {
			...
			var input, inputDelayed, delayTime, controlSignalEQ, controlSignalEQST, outSound;
			var openTimeSum, holdTimeSum, closeTimeSum, thresholdSum, hysteresisSum, reductionSum, mixSum;
			var compFunc, controlSignal, onThreshold, offThreshold, controlAmp, onTrig, offTrig, outGate, outGateHold;
			...
			if (monitorControl == true, {
				...
				controlSignalEQST = this.getSynthOption(5).value(controlSignalEQ);
				controlAmp = A2K.kr(Amplitude.ar(controlSignalEQST, 0.01, 0.01));
		...
		guiSpecArray = [
			...
			["SpacerLine", 1],
			["SynthOptionPopupPlusMinus", "Channel link", arrOptionData, 5],
			["SpacerLine", 1],
