...
TXCompander3St : TXModuleBase {		// Compander module
	...
	*initClass{
		...
		classData.defaultName = "Compander St";
	...
	init {arg argInstName;
		...
		arrOptionData = [
			[// 0 - Channel link
				...
				["Linked Left & Right channels", {
					arg input; max (abs(input[0]), abs(input[1]))}], // better
		...
		synthDefFunc = { arg in, sideChain, out, threshold, expanderRatio, expanderRatioMin, expanderRatioMax,
			...
			var compRatioSum, expRatio, controlInput, controlSignal, mixCombined, outSound;
			...
			controlInput = this.getSynthOption(0).value(input);
			controlSignal = this.getSynthOption(1).value(controlInput, sideChain);
		...
		guiSpecArray = [
			["SynthOptionPopupPlusMinus", "Channel link", arrOptionData, 0],
			["SpacerLine", 6],
			["SynthOptionPopupPlusMinus", "Side-chain", arrOptionData, 1],
			["SpacerLine", 6],
			["SynthOptionPopupPlusMinus", "Lookahead", arrOptionData, 2],
			["SpacerLine", 6],
