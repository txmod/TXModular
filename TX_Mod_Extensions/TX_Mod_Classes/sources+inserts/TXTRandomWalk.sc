...
TXTRandomWalk : TXModuleBase {
	...
	*initClass{
		...
		classData.defaultName = "T Random Walk";
		classData.moduleRate = "control";
		classData.moduleType = "source";
		classData.arrCtlSCInBusSpecs = [
			["Trigger", 1, "clockTrig", nil],
			["Reset", 1, "resetTrig", nil],
			["Max step size", 1, "modMaxStepSize", 0],
	...
	*new{ arg argInstName;
	...
	init {arg argInstName;
		...
		arrSynthArgSpecs = [
			...
			["maxStepSize", 0.1, defLagTime],
			["maxStepSizeMin", 0, defLagTime],
			["maxStepSizeMax", 1, defLagTime],
			["modMaxStepSize", 0, 0],
		];
		arrOptions = [0, 0, 1];
		arrOptionData = [
		...
		synthDefFunc = { arg out,
			...
			maxStepSize, maxStepSizeMin, maxStepSizeMax,
			modMaxStepSize = 0;

			var maxStepSizeSum, outLength, holdClockTrig, holdResetTrig, outTrig, outDist;
			var outDemand, rangeFunction, outSignal;

			maxStepSizeSum = (maxStepSize + modMaxStepSize).max(0).min(1)
				.linlin(0, 1, maxStepSizeMin, maxStepSizeMax);
			...
			outDemand = Demand.kr(outTrig, holdResetTrig,
				Dbrown(0, 1, maxStepSizeSum, outLength)
			);
		...
		guiSpecArray = [
			...
			["DividingLine"],
			["SpacerLine", 10],
			["TXMinMaxSliderSplit", "Max step size", ControlSpec(0, 1),
				"maxStepSize", "maxStepSizeMin", "maxStepSizeMax"],
			["DividingLine"],
			["SpacerLine", 10],
